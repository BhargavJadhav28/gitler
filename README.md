...
Examples use `v0.1.3`; replace it with the exact release you want.
...
--modify-path         Require adding the directory to a supported shell profile
...
`-AddToPath`, `-NoAddToPath`, `-RemovePath`, `-Uninstall`, and `-WhatIf`.

- Unix default directory: `${XDG_DATA_HOME:-$HOME/.local/share}/gitler/bin`.
- Windows default directory: `%LOCALAPPDATA%\Programs\gitler`.
- Normal installs automatically configure PATH when the platform and shell
  support it. Use `--no-modify-path` or `-NoAddToPath` to opt out. If automatic
  setup is unavailable, the installer prints copy-paste commands for the current
  terminal and future sessions.
...
```sh
gh attestation verify ./gitler-v0.1.3-linux-x86_64-gnu \
